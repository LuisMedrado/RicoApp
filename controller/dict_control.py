...
def insert_artigo_investimento_iniciante():
    ...
    if conn and cursor:
        try:
            artigo_investimento_1 = """INSERT INTO artigos (id, título, autor_artigo, data_artigo, conteudo_artigo)
...
def insert_artigo_renda_fixa_digital():
    ...
    if conn and cursor:
        try:
            artigo_investimento_2 = """INSERT INTO artigos (id, título, autor_artigo, data_artigo, conteudo_artigo)
...
def insert_artigo_criptomoedas():
    ...
    if conn and cursor:
        try:
            artigo_investimento_3 = """INSERT INTO artigos (id, título, autor_artigo, data_artigo, conteudo_artigo)
...
def insert_artigo_fiis():
    ...
    if conn and cursor:
        try:
            artigo_investimento_4 = """INSERT INTO artigos (id, título, autor_artigo, data_artigo, conteudo_artigo)
...
def carregar_artigos():
    try:
        ...
        print(f"Erro ao conectar ao banco: {e}")
        # aqui vai retornar true apenas por lógica. toda saída diferente de true vai ser interpretada como false, entao pra conseguir pegar essa saída na função vou deixar a saída certa como false
        # ou seja, o "return saida_artigos" um pouco pra baixo
        return True
    if conn and cursor:
        try:
            str_query = """SELECT * FROM artigos"""
            cursor.execute(str_query)
            saida_artigos = cursor.fetchall()
            print("Valores da tabela artigo recuperados.")
            # bem aqui
            return saida_artigos
        except sqlite3.Error as e:
...
def carregar_artigo_por_id(id_artigo):

    conn, cursor = get_db_connection()

    cursor.execute("SELECT * FROM artigos WHERE id = ?", (id_artigo,))
